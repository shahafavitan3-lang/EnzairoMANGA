...
const translations = {
  ...
  en: {
    ...
    toast_volume_created: 'Volume created',
...
const STORAGE_KEY = 'enzairo_site_data_v5';
const STORAGE_RESET_KEY = 'enzairo_clean_start_v5';

// Start this release with a genuinely empty site dataset. The one-time
// cleanup removes data saved by previous Enzairo/Kazan builds (including
// uploaded base64 images and old demo volumes), then future saves use v4.
function clearLegacyStorageOnce() {
  if (localStorage.getItem(STORAGE_RESET_KEY) === '1') return;

  const keysToRemove = [];
  for (let i = 0; i < localStorage.length; i++) {
    const key = localStorage.key(i);
    if (key && (key.startsWith('enzairo_') || key.startsWith('kazan_'))) {
      keysToRemove.push(key);
    }
  }
  keysToRemove.forEach(key => localStorage.removeItem(key));
  sessionStorage.removeItem('enzairo_admin_authenticated');
...
function renderHero() {
  ...
  heroImage.classList.toggle('hidden', !siteData.hero.image);
}

function renderAbout() {
  document.getElementById('aboutText').textContent = localizedContent(siteData.about.text);
  const aboutImage = document.getElementById('aboutImage');
  aboutImage.src = siteData.about.image || '';
  aboutImage.classList.toggle('hidden', !siteData.about.image);
}
...
function initAdminTabs() {
  ...
  tabs.forEach(tab => {
    tab.addEventListener('click', () => {
      ...
      if (tab.dataset.tab === 'volumes') renderAdminVolumesList();
    });
  });
}
  ...
  container.innerHTML = siteData.volumes
    ...
    .map(vol => `
      <div class="admin-volume-row">
        <img src="${vol.cover}" alt="${escapeHtml(localizedContent(vol.title))}" />
...
function initVolumeForm() {
  ...
  document.getElementById('volumeForm').addEventListener('submit', (e) => {
    ...
    try {
      ...
      const wasCreating = !editingVolumeId;
      ...
      renderAdminVolumesList();         // keep Existing Volumes in Admin in sync
      if (saved) showToast(t(wasCreating ? 'toast_volume_created' : 'toast_volume_saved'));
      renderAdminVolumesList();
      resetVolumeForm();
    } catch (err) {
