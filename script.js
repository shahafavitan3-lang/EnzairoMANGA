...
const LANG_STORAGE_KEY = 'enzairo_language';
let currentLang = localStorage.getItem(LANG_STORAGE_KEY) === 'en' ? 'en' : 'he';
...
function applyLanguage(lang) {
  ...
  // Re-render every public section that contains editable/dynamic text too.
  renderHero();
  renderAbout();
  renderVolumes();
  renderReviews();
  ...
  if (detailsVolumeId) {
    const activeVolume = siteData?.volumes?.find(v => v.id === detailsVolumeId);
    if (activeVolume) openVolumeDetails(activeVolume);
  }
}
