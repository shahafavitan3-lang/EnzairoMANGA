...
/* ---- Firebase (Realtime Database) ----
   Gives the site a real shared backend: admin edits and review
   submissions now sync to EVERY visitor, not just the browser
   that made them (previously everything lived only in that one
   browser's IndexedDB). IndexedDB stays in place as a fast local
   cache and an offline fallback — see loadSiteData/saveSiteData. */
const firebaseConfig = {
  apiKey: "AIzaSyAGctPicYsR4Oh0OwrriN_wS_zzq_LOEh4",
  authDomain: "enzairo-web.firebaseapp.com",
  databaseURL: "https://enzairo-web-default-rtdb.firebaseio.com",
  projectId: "enzairo-web",
  storageBucket: "enzairo-web.firebasestorage.app",
  messagingSenderId: "592184917737",
  appId: "1:592184917737:web:d1478cc60f87c15ba57a78",
  measurementId: "G-HPQ5YQN07F"
};
...
try {
  ...
  console.error('[Enzairo] Firebase failed to initialize — falling back to local-only storage:', err);
}

const FB_DATA_PATH = 'site_data';
let suppressNextRemoteRender = false; // true right after OUR OWN write, so the echo doesn't double-render

// Listens for changes written by ANY client (this tab, another tab, another
// visitor, another device in the admin panel) and keeps this page's public
// sections in sync live, with no reload needed.
function initFirebaseSync() {
  if (!fbDb) return;
  fbDb.ref(FB_DATA_PATH).on('value', (snapshot) => {
    if (suppressNextRemoteRender) { suppressNextRemoteRender = false; return; }
    const remote = snapshot.val();
    if (!remote) return; // nothing written yet — loadSiteData() already seeded it
    siteData = normalizeSiteData(remote);
    idbSet(IDB_DATA_KEY, siteData).catch(() => {}); // keep local cache warm, non-blocking
    safeInit(renderHero, 'renderHero (remote update)');
    safeInit(renderAbout, 'renderAbout (remote update)');
    safeInit(renderVolumes, 'renderVolumes (remote update)');
    safeInit(renderReviews, 'renderReviews (remote update)');
  }, (err) => {
    console.error('[Enzairo] Firebase sync listener error:', err);
  });
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  safeInit(initFirebaseSync, 'initFirebaseSync'); // live-sync from Firebase after the initial load
...
const translations = {
  he: {
    ...
    toast_sync_error: 'נשמר במכשיר הזה בלבד — הסנכרון האונליין נכשל.',
  ...
  en: {
    ...
    toast_sync_error: 'Saved on this device only — online sync failed.',
...
async function loadSiteData() {
  // Firebase is the source of truth (shared across every visitor). Try it
  // first so a fresh browser sees the admin's real content immediately,
  // not the built-in defaults.
  if (fbDb) {
    try {
      const snapshot = await fbDb.ref(FB_DATA_PATH).once('value');
      const remote = snapshot.val();
      if (remote) {
        siteData = normalizeSiteData(remote);
        idbSet(IDB_DATA_KEY, siteData).catch(() => {}); // warm local cache, non-blocking
        return;
      }
    } catch (err) {
      // Offline, blocked, or rules deny read — fall through to the local
      // IndexedDB copy below so the site still works.
      console.error('[Enzairo] Failed to read from Firebase, falling back to local cache:', err);
    }
  }
...
async function saveSiteData() {
  ...
  if (fbDb) {
    try {
      suppressNextRemoteRender = true; // this write will echo back via the 'value' listener — skip that redundant re-render
      await fbDb.ref(FB_DATA_PATH).set(siteData);
    } catch (err) {
      suppressNextRemoteRender = false;
      console.error('[Enzairo] Failed to save to Firebase (change is only stored locally for now):', err);
